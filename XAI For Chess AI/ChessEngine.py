...
class GameState:
    ...
    def getValidMoves(self):
        ...
        # print(f"in_check: {self.in_check}, pins: {self.pins}, checks: {self.checks}")
    ...
    '''def getValidMovesFromPosition(self, row, col):
        """
        Get all valid moves for the piece at the specified position (row, col).
        """
        piece = self.board[row][col]
        moves = []
        if piece != "--":  # If there's a piece at the specified location
            piece_type = piece[1]
    ...
    def checkForPinsAndChecks(self):
        ...
        # print(f"Checking from position ({start_row}, {start_col}) for {'white' if self.white_to_move else 'black'} king")
        ...
        for j in range(len(directions)):
            ...
            for i in range(1, 8):
                ...
                if 0 <= end_row <= 7 and 0 <= end_col <= 7:
                    ...
                    # print(f"Checking square ({end_row}, {end_col}) with piece {end_piece}")
                    if end_piece[0] == ally_color and end_piece[1] != "K":
                        ...
                        if (0 <= j <= 3 and enemy_type == "R") or (4 <= j <= 7 and enemy_type == "B") or (
                            ...
                            if possible_pin == ():  # no piece blocking, so check
                                ...
                                # print(f"Check detected from ({end_row}, {end_col}) in direction {direction}")
                                break
                            else:  # piece blocking so pin
                                pins.append(possible_pin)
                                # print(f"Pin detected at {possible_pin}")
                                break
        ...
        for move in knight_moves:
            ...
            if 0 <= end_row <= 7 and 0 <= end_col <= 7:
                ...
                if end_piece[0] == enemy_color and end_piece[1] == "N":  # enemy knight attacking a king
                    ...
                    # print(f"Knight check detected from ({end_row}, {end_col})")
        # print(f"Final in_check: {in_check}, pins: {pins}, checks: {checks}")
